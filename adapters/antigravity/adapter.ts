/**
 * Antigravity (Gemini CLI) adapter for hstry — CLI-only v1
 *
...
const DEFAULT_ANTIGRAVITY_PATH = join(homedir(), '.gemini', 'tmp');
...
const adapter: Adapter = {
  info(): AdapterInfo {
    return {
      ...
      version: '1.0.0',
      defaultPaths: [DEFAULT_ANTIGRAVITY_PATH],
    };
  },

  async detect(path: string): Promise<number | null> {
    if (!isUnderCanonicalRoot(path, DEFAULT_ANTIGRAVITY_PATH)) {
      return null;
    }
