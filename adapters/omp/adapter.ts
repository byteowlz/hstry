/**
 * Oh My Pi adapter for hstry
 *
 * Parses OMP session logs stored as JSONL files in ~/.omp/agent/sessions
 * 
...
const DEFAULT_OMP_PATH = join(homedir(), '.omp', 'agent', 'sessions');
...
const adapter: Adapter = {
  info(): AdapterInfo {
    return {
      ...
      version: '1.0.0',
      defaultPaths: [DEFAULT_OMP_PATH],
    };
  },

  async detect(path: string): Promise<number | null> {
    // trx-gzfh defense in depth: omp only owns ~/.omp/agent/sessions.
    if (!isUnderCanonicalRoot(path, DEFAULT_OMP_PATH)) {
      return null;
    }
...
function buildPiFiles(conversations: Conversation[], opts: ExportOptions): ExportFile[] {
  ...
  for (const conv of conversations) {
    ...
    for (const msg of conv.messages) {
      ...
      const contentBlocks: OmpContentBlock[] = [];
      if (msg.content) {
...
function mapRoleToPi(role: Message['role']): OmpMessage['role'] {
  switch (role) {
...
function buildContentParts(content?: OmpContentBlock[], includeTools?: boolean): CanonPart[] | undefined {
...
function buildToolResultParts(msg: OmpMessage): CanonPart[] | undefined {
...
function extractContent(content?: OmpContentBlock[]): string {
...
function extractToolCalls(content?: OmpContentBlock[]): ToolCall[] | undefined {
