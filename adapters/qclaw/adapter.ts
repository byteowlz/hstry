/**
 * QClaw / OpenClaw adapter for hstry
 *
 * Parses QClaw agent session logs stored as JSONL files under ~/.qclaw/agents.
 * Format matches Pi (session header + message tree); skips trajectory/checkpoint files.
 */
...
const DEFAULT_QCLAW_PATH = join(homedir(), '.qclaw', 'agents');

function isQclawSessionFile(filePath: string): boolean {
...
const adapter: Adapter = {
  info(): AdapterInfo {
    return {
      ...
      displayName: 'QClaw',
      version: '1.0.0',
      defaultPaths: [DEFAULT_QCLAW_PATH],
    };
  },

  async detect(path: string): Promise<number | null> {
    if (!isUnderCanonicalRoot(path, DEFAULT_QCLAW_PATH)) {
      return null;
    }
...
async function findJsonlFiles(
  ...
  if (stats.isFile()) {
    if (isQclawSessionFile(path)) files.push(path);
    return files;
  }
  ...
  while (queue.length > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isFile() && isQclawSessionFile(entryPath)) {
