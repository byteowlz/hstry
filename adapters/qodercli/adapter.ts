/**
 * Qoder CLI adapter for hstry
 *
 * Parses Qoder CLI session logs stored as JSONL files in ~/.claude/projects
 */
...
const DEFAULT_QODER_PATH = join(homedir(), '.qoder', 'projects');
...
const adapter: Adapter = {
  info(): AdapterInfo {
    return {
      ...
      version: '1.0.0',
      defaultPaths: [DEFAULT_QODER_PATH],
    };
  },

  async detect(path: string): Promise<number | null> {
    // trx-gzfh defense in depth: qodercli only owns ~/.qoder/projects.
    ...
    if (!isUnderCanonicalRoot(path, DEFAULT_QODER_PATH)) {
      return null;
    }
...
function extractMessages(entries: JsonlEntry[]): Message[] {
  ...
  for (const entry of entries) {
    ...
    // Compact summaries are synthetic user messages injected by Qoder CLI on
...
/** Build CanonPart[] from Qoder CLI content blocks. */
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      for (const child of nested) {
        ...
        } else if (child.isDirectory() && child.name === 'transcript') {
          const transcriptDir = join(entryPath, child.name);
          const transcripts = await readdir(transcriptDir, { withFileTypes: true }).catch(() => []);
          for (const transcript of transcripts) {
            if (transcript.isFile() && extname(transcript.name) === '.jsonl') {
              files.push(join(transcriptDir, transcript.name));
            }
          }
        }
      }
